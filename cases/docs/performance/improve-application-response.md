...
![反例总时长](./images/improve-application-response-promise-all-duration.png)

反例中启动时aboutToAppear阶段耗时为295ms。

![反例时长](./images/improve-application-response-promise-duration.png)

正例中启动时总耗时为169ms。

![正例总时长](./images/improve-application-response-settimeout-all-duration.png)

正例中启动时aboutToAppear阶段耗时为167us。

![正例时长](./images/improve-application-response-settimeout-duration.png)

其中setTimeout真正的耗时为308ms。

![正例时长](./images/improve-application-response-settimeout-real-duration.png)

异步运行机制如图所示。

![异步运行机制](./images/improve-application-response-async.png)
...
![反例响应时延](./images/preload_counter_example_delay.png)

正例响应时延：9.4ms

![正例响应时延](./images/preload_positive_example_delay.png)
...
![骨架图占位](./images/improve-application-response-skeleton.png)

将使用和未使用骨架图的组件通过SmartPerf-Host工具抓取trace后对比可得：

未使用骨架图时，响应时间约为321.5ms。(其中包含setTimeout的300ms)

![骨架图占位](./images/improve-application-response-no-skeleton-duration.png)

使用了骨架图后，响应时间变为10.3ms。

![骨架图占位](./images/improve-application-response-skeleton-duration.png)
...
![反例响应时延](./images/pangesture_distance_max.png)

日志主要关注从应用接收TouchDown事件到pan识别耗时，该过程耗时127ms。（注：日志信息和trace图非同一时间获取，所获得的性能数据存在差异，提供的数值仅供参考。）

反例日志

![反例响应时延日志](./images/pangesture_distance_max_log.png)
...
![正例响应时延](./images/pangesture_distance_min.png)

日志主要关注从应用接收TouchDown事件到pan识别耗时，该过程耗时42ms。（注：日志信息和trace图非同一时间获取，所获得的性能数据存在差异，提供的数值仅供参考。）

正例日志

![正例响应时延日志](./images/pangesture_distance_min_log.png)
