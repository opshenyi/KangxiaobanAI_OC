...
![动画](images/highly-loaded-component-render-1.gif)

### 解决思路

既然转场时一次性加载大量的数据会导致卡顿情况，那么将数据拆分成多份并分批次进行加载就是一种解决思路。ArkTS中提供了[DisplaySync（可变帧率）](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-graphics-displaysync-0000001813575980)，可以设置帧回调监听，让开发者在不同的帧中进行一些操作，这样就可以将本来在一帧中加载的数据分到多帧中加载，减少动画首帧的响应时间，降低完成时延。

### 常规代码

在自定义列表组件中一次性加载全部数据，可参考[组件堆叠场景](../../CommonAppDevelopment/feature/componentstack)中的具体实现。
...
![image-20240717183013984](images/highly_loaded_component_transition_normal.png)
...
![image-20240717183924458](images/highly_loaded_component_transition_displaysync.png)
...
[示例代码](../../CommonAppDevelopment/feature/perfermance/highlyloadedcomponentrender)
