...
>**说明：**
>
> 1. 关于本文中示例，可参考：[提升应用冷启动速度示例](../../test/performance/startup/)。  
...
以下为示例代码：  
【优化前】存在多层嵌套export *的方式全量导出。[源码参考](../../test/performance/app_cold_start/RedundantExport/entry/src/main/ets/pages)
```ts
// Numbers.ets
...
```
【优化后】不存在嵌套export *，从目标文件中直接import。[源码参考](../../test/performance/app_cold_start/NormalExport/entry/src/main/ets/pages)
```ts
...
以下为示例代码：  
【优化前】Index.ets中使用 import * as nm from '../utils/Numbers'。[源码参考](../../test/performance/app_cold_start/RedudantImport/entry/src/main/ets)
```ts
// Index.ets
...
【优化后】Index.ets中使用 import { One } from '../utils/Numbers'。[源码参考](../../test/performance/app_cold_start/NormalImport/entry/src/main/ets)
```ts
// Index.ets
...
【优化前】加载模块时执行了非冷启动相关文件SubPage.ets。[源码参考](../../test/performance/app_cold_start/ImportIndexFromHar)
...
以下为示例代码：
1. 将HAR包的导出文件Index.ets进行拆分，IndexAppStart.ets文件仅导出首页相关文件，IndexOthers.ets文件导出非首页相关文件。[源码参考](../../test/performance/app_cold_start/ImportIndexAppStartFromHar/library)
```ts
...
```
2. 首页Index从IndexAppStart.ets导入MainPage。[源码参考](../../test/performance/app_cold_start/ImportIndexAppStartFromHar/entry/src/main/ets/pages/Index.ets)
```ts
// Index.ets
...
```
3. 跳转后的页面SecondPage从IndexOthers.ets导入SubPage。[源码参考](../../test/performance/app_cold_start/ImportIndexAppStartFromHar/entry/src/main/ets/pages/SecondPage.ets)
```ts
// SecondPage.ets
...
以下为示例代码，[源码参考](../../test/performance/app_cold_start/ImportExpandPathFromHar/library/src/main/ets/components/mainpage/MainPage.ets) ：
```ts
// Index.ets
...
以下为示例代码：
1. 在被引用HAR_COMMON包中写入功能示例。[源码参考](../../test/performance/app_cold_start/HapAndHspDependHar/har_common/src/main/ets/utils/Utils.ets)
```ts
...
- 使用HAP包和HSP包引用该HAR_COMMON包中的功能。  
  HAP包引用HAR_COMMON包中的功能。[源码参考](../../test/performance/app_cold_start/HapAndHspDependHar/entry/src/main/ets/pages/Index.ets)
  ...
  ```
  HSP包引用HAR_COMMON包中的功能。[源码参考](../../test/performance/app_cold_start/HapAndHspDependHar/hsp_library/src/main/ets/pages/MainPage.ets)
  ```ts
  // hsp_library/src/main/ets/pages/MainPage.ets
  import { funcResult } from 'har_common';
  ```
- 使用HAP包和HAR包引用该HAR_COMMON包中的功能。
  HAP包引用HAR_COMMON包中的功能。[源码参考](../../test/performance/app_cold_start/HapAndHarDependHar/entry/src/main/ets/pages/Index.ets)
  ```ts
  // entry/src/main/ets/pages/Index.ets
  import { MainPage } from 'har_library';
  import { funcResult } from 'har_common';
  ```
  HAR包引用HAR_COMMON包中的功能。[源码参考](../../test/performance/app_cold_start/HapAndHarDependHar/har_library/src/main/ets/pages/MainPage.ets)
  ```ts
...
以下为示例代码：  
【优化前】：在首页根组件的onAppear()生命周期回调中发起网络请求。[源码参考](../../test/performance/app_cold_start/HttpRequest/entry/src/main/ets)
```ts
// entry/src/main/ets/pages/Index.ets
...
【优化后】
1. 在NetRequest.ets中进行Http请求以及数据处理。[源码参考](../../test/performance/app_cold_start/PreHttpRequest/entry/src/main/ets/utils/NetRequest.ets)
```ts
// NetRequest.ets
import { hiTraceMeter } from '@kit.PerformanceAnalysisKit';
import { http } from '@kit.NetworkKit';
...
```
2. 在AbilityStage的onCreate()生命周期回调中发起网络请求。[源码参考](../../test/performance/app_cold_start/PreHttpRequest/entry/src/main/ets/myabilitystage/MyAbilityStage.ets)
```ts
// MyAbilityStage.ets
...
```
3. 在首页Index.ets中展示请求获取的图片。[源码参考](../../test/performance/app_cold_start/PreHttpRequest/entry/src/main/ets/pages/Index.ets)
```ts
// Index.ets
import { number } from '../utils/Calculator';
