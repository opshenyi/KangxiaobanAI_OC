...
建议开发者在Web页面启动前执行预启动Web渲染进程方案，如应用冷启动或者广告阶段，如果无法再冷启动期间进行Web渲染进程预启动，建议空闲时间预启动Web渲染进程。


**Web渲染进程简介**

Web渲染进程是用于Web页面渲染而拉起的进程，应用内部有且仅有一个Web渲染进程存活。Web页面渲染时会根据Web渲染进程的状态判断是否需要拉起Web渲染进程。当所有Web组件全部销毁时，Web渲染进程才会被杀掉。

> 说明  
> 
> 1.该方案通过创建一个空白的ArkWeb组件来预启动Web渲染进程，额外创建ArkWeb组件会消耗内存、算力，预创建一个空白的ArkWeb组件约消耗200Mb内存，所以建议后续页面加载服用预创建的ArkWeb组件。
> 
...
点击“加载下一页”按钮，页面加载按成耗时如下图所示：

![](./figures/performance-web-import-un-pre-startup.png)


【推荐用法】

在后台创建一个ArkWeb组件来预先启动用于渲染的Web渲染进程，具体步骤如下：
...
2. 创建载体，并创建ArkWeb组件，加载一个blank页面即可
...
![](./figures/performance-web-import-pre-startup.png)

> 说明
> 建议开发者复用预创建的Web组件

[预启动Web渲染进程Demo工程](../../test/performance/web_import/WebImportRender)
...
[提前初始化Web内核Demo工程](../../test/performance/web_import/WebImportPreStartWebEngine)
...
[预解析和预连接Demo工程](../../test/performance/web_import/WebImportPreConnect)
...
[预下载Demo工程](../../test/performance/web_import/WebImportPreDownload)
...
3. 通过NodeContainer使用已经预渲染的页面
    ...
    ```
[预渲染Demo工程](../../test/performance/web_import/WebImportRender)
    ...
    Column() {
      Button('runJavaScript')
        .onClick(() => {
          ...
          // 前端页面函数无参时，将param删除。
          this.webviewController.runJavaScript('htmlTestWithParam(param)');
        })
...
前端页面代码：
```html
// html文件路径：src/main/resources/rawfile/index.html
<!DOCTYPE html>
<html>
<body>
...
<script>
  ...
  function htmlTestWithParam(param) {
      ...
      Row() {
        Web({ src: $rawfile('runJS.html'), controller: this.controller })
          ...
          .onControllerAttached(() => {
            console.info("ndk onControllerAttached webId: " + this.controller.getWebId());
          })
      }.height('80%')
    }
  }
...
```C
// hello.cpp文件路径：/src/main/cpp/hello.cpp
...
```html
// html文件路径：src/main/resources/rawfile/runJS.html
<!DOCTYPE html>
<html lang="en-gb">
<head>
...
[NDK接口实现JSBridge通信Demo工程](../../test/performance/web_import/WebImportJSBridge)
...
[预编译JavaScript生成字节码缓存Demo工程](../../test/performance/web_import/WebImportPreCompileJS)
...
资源拦截替换的JavaScript生成字节码缓存适用于在页面非首次加载时存在需要加载网络JavaScript文件并可以进行拦截替换的场景，支持其生成字节码缓存到本地，在页面非首次加载时节省编译时间。

Web组件默认支持HTTP协议的JavaScript生成字节码缓存，也支持自定义协议的JavaScript生成字节码缓存。具体操作步骤如下：
...
不设置ResponseDataID，直接加载Web页面，具体步骤如下：
...
2. 不设置ResponseDataID，进行页面请求拦截替换
...
![](./figures/performance-web-import-un-set-response-id.png)


【推荐用法】

在进行资源拦截替换时，请求头中设置ResponseDataID
...
![](./figures/performance-web-import-set-response-id.png)


**场景二 调用ArkTS接口， webview.WebviewController.customizeSchemes(schemes: Array\<WebCustomScheme>): void**
      ...
      Flex({ direction: FlexDirection.Column, alignItems: ItemAlign.Center, justifyContent: FlexAlign.Center }) {
        Web({
          ...
          .onInterceptRequest(event => {
            ...
            // 拦截页面请求
            if (event?.request.getRequestUrl() === 'scheme://www.example.com/test.js') {
...
**场景三 调用Native接口，int32_t OH_ArkWeb_RegisterCustomSchemes(const char * scheme, int32_t option)**
...
[资源拦截替换的JavaScript生成字节码缓存Demo工程](../../test/performance/web_import/WebImportInterCustomerJS)

**总结（以拦截HTTP协议的JavaScript生成字节码缓存场景性能数据举例）**
...
[异步JSBridge调用Demo工程](../../test/performance/web_import/WebImportSyncJSBridge/)
...
常规案例使用懒加载的逻辑加载图片，图片组件进入可视区域后再执行加载，滑动过程中列表有大量图片未加载完成产生的白块。[源码链接](../../test/performance/web_slide_white_block/WebSlideWhiteBlockNegative/entry/src/main/resources/rawfile/img_list.html)
...
根据上方公式，优化案例设定在400mm/s的速度滑动屏幕，此时可计算应用需预加载0.5个屏幕高度的图片。在常规加载案例中，页面将可视窗口作为根元素，rootMargin属性均为0，可视窗口与设备屏幕高度相等。此时可通过设置`rootMargin`向下方向为50%（即0.5个屏幕高度），扩展可视窗口的高度，使图片在屏幕外提前进入可视窗口。当图片元素进入可视窗口时，会将img标签的data-src属性中保存的图片地址赋值给src属性，从而实现图片的预加载。应用会查询页面上所有具有data-src属性的img标签，并开始观察这些图片。当某张图片进入已拓展高度的可视窗口时，就会执行相应的加载操作，实现页面预渲染更多图片，解决滑动白块问题。[源码链接](../../test/performance/web_slide_white_block/WebSlideWhiteBlockPositive/entry/src/main/resources/rawfile/img_list.html)
