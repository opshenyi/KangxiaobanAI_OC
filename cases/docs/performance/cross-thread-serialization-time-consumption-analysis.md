...
通常情况下，单个图书信息会被定义成class Book，通过组装该实例填充数据，最后将包含所有Book实例的BookDBInfo对象通过Taskpool传递给子线程。[源码参考](../../test/performance/cross_thread_serialization_time_consumption_analysis/entry/src/main/ets/pages/NotUseSendable.ets)
...
为了解决该场景的序列化超时问题，将上述示例中dbInfo相关的class进行[Sendable改造](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides-V5/arkts-sendable-V5)，将单个书本信息的类型定义为Sendable类型，并改造内部成员属性类型为Sendable类型。
