...
1. 在需要使用的页面引入high_light_guide库中的引导页组件。
   ...
   ```typescript
   // 引入引导页组件
   import { HighLightGuideBuilder, HighLightGuideComponent, Controller, GuidePage, HighLightShape, RectF } from 'high_light_guide';
   ```
...
3. 设定各setHighLightIndicator里的引导层布局组件
   ...
   4. 计算id为test组件处于页面中的位置，为引导说明组件做位置控制。
      ...
      // 此案例x坐标为中心点到id为test组件宽度的一半的距离
      // y坐标为id为test组件相对于页面顶部的距离减去Indicator自身高度的值
...
[动态路由模块](../../common/routermodule/src/main/ets/router/DynamicsRouter.ets)实现页面动态加载
