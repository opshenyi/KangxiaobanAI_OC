...
```typescript
"dependencies": {
  "votingcomponent": "har包地址"
}
```
2. ets文件import自定义视图实现列表视图。

```typescript
import { VotingView, VotingType } from 'votingcomponent';
```

### 快速使用

本章节主要介绍了如何快速实现一个投票PK的功能。
1. 初始化投票PK内容数据。开发者可以自行传递VotingType的属性值。
...
export class VotingType {
  ...
  constructor(leftOptionChoose: number = 4, rightOptionChoose: number = 13, leftOption: string = Constants.LEFT_OPTION,
...
|        属性         |   类型   |    释义    | 默认值 |
|:-----------------:|:------:|:--------:|:---:|
| leftOptionChoose  | number | 左边选项投票人数 |  4  |
| rightOptionChoose | number | 右边选项投票人数 | 13  |
|    leftOption     | string |  左边选项文本  | 早起  |
|    rightOption    | string |  右边选项文本  | 晚起  |

VotingView视图属性

|      属性      |     类型     |   释义   | 默认值 |
|:------------:|:----------:|:------:|:---:|
| votingSource | VotingType | 投票资源属性 |  -  |
| votingHandle |    void    |  投票逻辑  |  -  |


### 实现思路
1. 使用Column绘制胶囊块图形，为实现胶囊块的切割效果以及投票后的比例变换，这里使用两个Column分别绘制两个半个胶囊块拼接而成。源码参考[VotingView.ets](src/main/ets/utils/VotingView.ets)。
...
2. 使用绘制组件中的Polygon实现胶囊块中间被分割的效果。中间的间隙有两个状态：1、未投票时是个宽度比较大的平行四边形，且有PK两字；2、投票后是个宽度很窄的平行四边形，且PK两字消失。源码参考[VotingView.ets](src/main/ets/utils/VotingView.ets)。
...
3. 计算投票比例作为左右图形宽度，且定义动画效果。源码参考[VotingView.ets](src/main/ets/utils/VotingView.ets)。
