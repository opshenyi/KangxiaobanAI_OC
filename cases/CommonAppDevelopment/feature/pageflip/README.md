...
#### 场景四： TextReader朗读控件+设置监听播放面板状态变化与播放的文章状态变更的回调函数实现文本朗读。
实现步骤：
   1. 初始化TextReader朗读控件。
   2. 设置监听，在用户与控件进行交互时触发回调。
   <br>源码参考:<br>[BottomView.ets](./src/main/ets/view/BottomView.ets)。
...
[**routermodule**](../../common/routermodule)
